...
int utils::parse_args(int argc, char ** argv, model * pmodel) {
    ...
    while (i < argc) {
	    ...
	    } else if (arg == "-greedysampling") {
	        greedy_sampling = atoi(argv[++i]);
	    } else if (arg == "-lang") {
    ...
    if (model_status == MODEL_STATUS_EST) {
        ...
        if (greedy_sampling >= 0 && greedy_sampling <= 1) {
            pmodel->greedy_sampling = (greedy_sampling != 0);
        }
	    ...
	    string::size_type idx2 = dfile2.find_last_of("/");			
	    if (idx2 == string::npos) {
	        pmodel->dir2 = "./";
	    } else {
	        pmodel->dir2 = dfile2.substr(0, idx2 + 1);
	        pmodel->dfile2 = dfile2.substr(idx2 + 1, dfile2.size() - pmodel->dir2.size());
	        printf("dir2 = %s\n", pmodel->dir2.c_str());
	        printf("dfile2 = %s\n", pmodel->dfile2.c_str());
        }
        if (train_method == 2) {
            string filename = pmodel->dir + "model-final-1" + pmodel->theta_suffix;
            FILE * fin = fopen(filename.c_str(), "r");
            if (fin) {
	            // read <model>.others file to assign values for ntopics, alpha, beta, etc.
	            if (read_and_parse(pmodel->dir + "model-final-1" + pmodel->others_suffix, pmodel)) {
	                return 1;
	            }	
            }
        }
